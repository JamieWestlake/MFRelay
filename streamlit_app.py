...
# --- New hand ---
def new_hand():
    with st.spinner('🌀 Shuffling new hand...'):
        time.sleep(0.5)
    st.session_state.random_index = random.choice(filtered_indices)
    st.session_state.submitted = False
    st.session_state.start_time = time.time()
    ...
    if submit:
        ...
        if "Bidding Info" in selected_sections:
            ...
            if user_input and len(user_input) == 4 and user_input.isdigit():
                ...
                distribution_ok = any(user_dist == dist and user_type.lower() == typ for dist, typ in possible)
            else:
        ...
        if correct_sections and not incorrect_sections:
            ...
            st.error("❌ Some sections incorrect.")
            if correct_sections:
                st.success(f"✅ Correct: {', '.join(correct_sections)}")
            if incorrect_sections:
                st.error(f"❌ Incorrect: {', '.join(incorrect_sections)}")
...
# --- Next Hand Button Centered ---
if st.session_state.submitted:
    left_space, button_col, right_space = st.columns([2, 1, 2])
    with button_col:
        st.button("▶️ Next Hand", use_container_width=True, on_click=new_hand)
